...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("VNExpressBot.log", encoding='utf-8'),
...
def parse_articles(html):
    ...
    # Extract articles from the main list on the homepage
    for item in soup.find_all('article', class_='item-news'):
        title = item.find('h3', class_='title-news').get_text(strip=True)
        link = item.find('a')['href']
        summary_tag = item.find('p', class_='description')
        summary = summary_tag.get_text(strip=True) if summary_tag else "No summary available"

        # Check and add only new articles
            ...
            while True:
                ...
                tasks = [
                    send_message_via_telegram(
                        ...
                        f"<b>Link:</b> {link}"
                    ) for title, link, summary in articles[:5]
                ]

                if tasks:
                    await asyncio.gather(*tasks)

                logging.info("Sleeping for 10 minutes before the next check")
